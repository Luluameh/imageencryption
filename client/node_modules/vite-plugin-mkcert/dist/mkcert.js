...
var resolvePath = (fileName) => {
  return import_path2.default.resolve(PLUGIN_DATA_DIR, fileName);
};
...
var Config = class {
  ...
  async merge(obj) {
    ...
    debug(`Receive parameter ${prettyLog(obj)}, then update config from ${currentStr} to ${nextStr}`);
...
var Mkcert = class {
  ...
  constructor(options) {
    ...
    this.mkcertSavedPath = resolvePath(import_process.default.platform === "win32" ? "mkcert.exe" : "mkcert");
  ...
  async checkMkcert() {
    ...
    if (this.mkcertLocalPath) {
      ...
      this.logger.error(import_picocolors.default.red(`${this.mkcertLocalPath} does not exist, please check the mkcertPath paramter`));
    } else {
  ...
  async createCertificate(hosts) {
    ...
    const cmd = `${escape(mkcertBinnary)} -install -key-file ${escape(KEY_FILE_PATH)} -cert-file ${escape(CERT_FILE_PATH)} ${names}`;
    await exec(cmd, {
      env: {
...
${CERT_FILE_PATH}`);
  ...
  async upgradeMkcert() {
    ...
    if (versionInfo.breakingChange) {
      debug("The current version of mkcert is %s, and the latest version is %s, there may be some breaking changes, update skipped", versionInfo.currentVersion, versionInfo.nextVersion);
      return;
    }
  ...
  async renew(hosts) {
    ...
    if (record.tamper(hash)) {
      debug(`The hash changed from ${prettyLog(record.getHash())} to ${prettyLog(hash)}, start regenerate certificate`);
      await this.regenerate(record, hosts);
      return;
    }
