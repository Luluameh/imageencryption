          ...
          var GenericGFPoly = function() {
            ...
            GenericGFPoly2.prototype.evaluateAt = function(a) {
              ...
              for (var i2 = 1; i2 < size; i2++) {
                result = GenericGF_1.addOrSubtractGF(this.field.multiply(a, result), this.coefficients[i2]);
              }
              return result;
            };
          ...
          function binarize(data, width, height, returnInverted) {
            ...
            for (var x = 0; x < width; x++) {
              for (var y = 0; y < height; y++) {
                var r2 = data[(y * width + x) * 4 + 0];
                var g = data[(y * width + x) * 4 + 1];
                var b = data[(y * width + x) * 4 + 2];
                greyscalePixels.set(x, y, 0.2126 * r2 + 0.7152 * g + 0.0722 * b);
              }
            }
          ...
          function readCodewords(matrix, version, formatInfo) {
            ...
            for (var columnIndex = dimension - 1; columnIndex > 0; columnIndex -= 2) {
              ...
              for (var i2 = 0; i2 < dimension; i2++) {
                var y = readingUp ? dimension - 1 - i2 : i2;
...
var n = (e2, t2, o2) => e2.length > 0 ? e2[0].deviceId : t2.length === 1 || o2 === "user" ? t2[0].deviceId : t2[1].deviceId;
var r = ({ onError: r2 = (e2) => console.log({ error: e2 }), onScan: a = (e2) => console.log({ value: e2 }), facingMode: s = "environment", constraints: c = null, onLoad: d = () => null, flipHorizontally: l = false, style: v, className: m, showViewFinder: g = true, delay: h = 800, resolution: u = 600, video: p = { width: "50%", height: "85%" }, viewFinder: f = { border: "12px solid rgba(255,255,255,0.3)", position: "absolute", borderRadius: "5px", width: "250px", height: "250px" } }) => {
  const w = {};
  let b, y;
  (0, import_react.useEffect)(() => (E(), () => {
    b && clearTimeout(b), y && y();
  }), []);
  const E = () => {
    const e2 = /firefox/i.test(navigator.userAgent);
    let t2 = {};
    navigator.mediaDevices && typeof navigator.mediaDevices.getSupportedConstraints == "function" && (t2 = navigator.mediaDevices.getSupportedConstraints());
    const o2 = {};
    t2.facingMode && (o2.facingMode = { ideal: s }), t2.frameRate && (o2.frameRate = { ideal: 25, min: 10 });
    const a2 = t2.facingMode || e2 ? Promise.resolve(c || o2) : ((e3, t3 = n) => new Promise((o3, n2) => {
      let r3;
      try {
        r3 = navigator.mediaDevices.enumerateDevices();
      } catch (e4) {
        n2(new i());
      }
      r3.then((r4) => {
        const a3 = r4.filter((e4) => e4.kind === "videoinput");
        if (a3.length < 1)
          return void n2(new i());
        const s2 = ((e4) => e4 === "environment" ? /rear|back|environment/gi : /front|user|face/gi)(e3), c2 = a3.filter(({ label: e4 }) => s2.test(e4));
        o3(t3(c2, a3, e3));
      });
    }))(s).then((e3) => Object.assign({}, { deviceId: e3 }, c));
    a2.then((e3) => navigator.mediaDevices.getUserMedia({ video: e3 })).then(L).catch(r2);
  }, L = (e2) => {
    const { preview: t2 } = w;
    if (!t2)
      return setTimeout(() => L(e2), 200);
    (t2 || {}).srcObject !== void 0 ? t2.srcObject = e2 : t2.mozSrcObject !== void 0 ? t2.mozSrcObject = e2 : window.URL.createObjectURL ? t2.src = window.URL.createObjectURL(e2) : window.webkitURL ? t2.src = window.webkitURL.createObjectURL(e2) : t2.src = e2, t2.playsInline = true;
    const o2 = e2.getTracks()[0];
    y = o2.stop.bind(o2), t2.addEventListener("loadstart", () => j(t2, o2.label));
  }, j = (e2, t2) => {
    e2.play(), d({ mirrorVideo: s === "environment", streamLabel: t2 }), b = setTimeout(R, h), e2.removeEventListener("loadstart", j);
  }, R = () => {
    const { preview: e2, canvas: t2 } = w;
    let i2 = Math.floor(e2.videoWidth), n2 = Math.floor(e2.videoHeight), r3 = 0, s2 = 0;
    const c2 = u / (i2 < n2 ? i2 : n2);
    n2 *= c2, i2 *= c2, s2 = (n2 - u) / 2 * -1, r3 = (i2 - u) / 2 * -1, t2.width = u, t2.height = u;
    if (e2 && e2.readyState === e2.HAVE_ENOUGH_DATA) {
      const c3 = t2.getContext("2d"), d2 = () => {
        c3.drawImage(e2, r3, s2, i2, n2);
        ((e3) => new Promise((t3) => {
          const i3 = (0, import_jsqr.default)(e3.data, e3.width, e3.height);
          i3 ? t3(i3.data) : t3();
        }))(c3.getImageData(0, 0, t2.width, t2.height)).then((e3) => {
          b = setTimeout(d2, h), e3 && a(e3);
        });
      };
      d2();
    } else
      b = setTimeout(R, h);
  }, D = (e2) => (t2) => w[e2] = t2, I = __spreadProps(__spreadValues({}, p), { backgroundColor: "red", overflow: "hidden" }), O = { top: 0, left: 0, right: 0, bottom: 0, margin: "auto auto auto auto" }, U = __spreadProps(__spreadValues(__spreadValues({}, O), p), { display: "block", position: "absolute", overflow: "hidden", objectFit: "cover", transform: l ? "scaleX(1)" : "scaleX(-1)" }), x = __spreadProps(__spreadValues(__spreadValues({}, O), f), { zIndex: 10, position: "absolute" });
  return import_react.default.createElement("section", { className: m, style: v }, import_react.default.createElement("section", { style: I }, g ? import_react.default.createElement("div", { style: x }) : null, import_react.default.createElement("video", { style: U, ref: D("preview") }), import_react.default.createElement("canvas", { style: { display: "none" }, ref: D("canvas") })));
};
export {
  r as QrScanner
};
//# sourceMappingURL=react-qrcode-scanner.js.map
